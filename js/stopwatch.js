...
export const sw = {
    ...
    formatTime(ms, forceMs = null) {
        ...
        const m = Math.floor(ms / 60000), s = Math.floor((ms % 60000) / 1000), milli = Math.floor((ms % 1000) / 10);
        let str = `${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`;
        if (showMs) str += `.${String(milli).padStart(2, '0')}`;
        return str;
    },
    ...
    openNameModal(action, defaultName, targetId = null) {
        ...
        if (this.els.nameTitle) {
            this.els.nameTitle.textContent = action === 'rename' ? t('rename') : t('save_session');
        }
        ...
        this.els.nameModalContent.classList.remove('opacity-0', 'translate-y-12');
    ...
    closeNameModal() {
        ...
        this.els.nameModalContent.classList.add('opacity-0', 'translate-y-12');
    ...
    renderSavedSessions() {
        ...
        this.savedSessions.forEach(session => {
            ...
            div.innerHTML = `
                <div class="p-4 cursor-pointer flex justify-between items-center active:bg-gray-500/10" onclick="sw.toggleSessionDetails(${session.id})">
                ...
                <div id="sw-details-${session.id}" class="hidden bg-black/5 dark:bg-black/20 border-t app-border p-4">
                    <div class="flex justify-end gap-2 mb-3">
                        <button class="px-3 py-1 bg-blue-500/10 text-blue-500 rounded-lg text-xs font-bold uppercase tracking-wider active:scale-95 transition-transform" onclick="sw.prepareRenameSession(${session.id}, event)">${t('rename')}</button>
                        <button class="px-3 py-1 bg-red-500/10 text-red-500 rounded-lg text-xs font-bold uppercase tracking-wider active:scale-95 transition-transform" onclick="sw.deleteSession(${session.id}, event)">${t('delete')}</button>
                    </div>
